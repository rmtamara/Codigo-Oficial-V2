...
file = 'base.xlsx'
base_mes = pd.read_excel(file, sheet_name='Hoja6')
...
caudal = open('Abril2/caudal_abr2.pkl', 'rb')
m_htf = pickle.load(caudal)
tiempo = open('Abril2/tiempo_abr2.pkl', 'rb')
time = pickle.load(tiempo)
temp_sf = open('Abril2/temp_abr2.pkl', 'rb')
u = pickle.load(temp_sf)
tiempo2 = open('Abril2/tiempo2_abr2.pkl', 'rb')
time2 = pickle.load(tiempo2)
temp_sgs = open('Abril2/temp_sgs_abr2.pkl', 'rb')
u2 = pickle.load(temp_sgs)
power = open('Abril2/power_abr2.pkl', 'rb')
Qpb = pickle.load(power)
work = open('Abril2/work_abr2.pkl', 'rb')
Wt = pickle.load(work)
msup = open('Abril2/q_sup_abr2.pkl', 'rb')
...
ax1.plot(time/3600, m_htf, color=color, linewidth = 2)
...
ax2.plot(time/3600, celsius(u[:, 0]), color=color, linewidth = 2)
...
ax3.plot(time/3600, fit_rad(time/3600), color=color, linewidth = 2)
...
ax4.plot(time/3600, celsius(u[:, 0]), color=color, linewidth = 2)
...
ax5.plot(time2/3600, Wt[1:len(Wt)]/10**6, color=color, linewidth = 2)
...
ax6.plot(time2/3600, Qpb[1:len(Qpb)]/10**6, color=color, linewidth = 2)
...
for attribute, measurement in bar.items():
    offset = width * multiplier
    rects = ax13.bar(x + offset, measurement, width, label = attribute)
    ax13.bar_label(rects, padding=2)
    multiplier += 1

ax13.set_xticks(x + width, hr)
plt.show()
"""

fig, ax14 = plt.subplots()
rects = ax14.bar(hr, htf.values(), width=0.5, edgecolor="white", linewidth=0.7)
ax14.bar_label(rects, padding=2)
ax14.set_ylabel('Caudal [kg/s]', fontsize = 30)
...
rects = ax15.bar(hr, w.values(), width=0.5, edgecolor="white", linewidth=0.7)
ax15.bar_label(rects, padding=2)
